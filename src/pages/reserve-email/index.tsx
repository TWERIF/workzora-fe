...
export default function ReserveEmail() {
  ...
  const validate = () => {
    ...
    if (!submit)
      tmpErrors[2] = t('auth.errors.submitRules');
        ...
        <div className="flex-1 max-w-[50%] bg-bg opacity-1 z-10 flex flex-col items-end justify-center dark:bg-bg-dark">
          <div className="flex flex-col items-center gap-[30px] mr-[77px]">
            ...
            <div className="flex flex-col mt-[25px] w-full max-w-[541px] items-center">
              <span className="font-semibold text-text dark:text-text-dark">{t("auth.texts.welcome")}</span>
              <span className="text-text dark:text-text-dark mt-[11px] mb-[13px]">{t("auth.texts.welcomeText")}</span>
                ...
                <Submit
                  ...
                  text={
                    <span className="whitespace-normal break-words">
                      {t("auth.texts.agree")}
                      <Link href="/agreement" text={" " + t("auth.texts.user") + " "} />
                      {t("auth.texts.and")}
                      <Link href="/privacy" text={" " + t("auth.texts.privacy")} />
                    </span>
                  }
                />
              </div>
              <ButtonGradient onClick={addEmail} text={t("auth.buttons.join")} />
            </div>
          </div>
        </div>
      </AuthLayout>
