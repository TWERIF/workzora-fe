...
import { PortfolioItem } from "../model/types"; // уточніть ваш шлях до типів

interface PortfolioModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: FormData) => void; // Тепер передаємо FormData
...
export default function PortfolioModal({ isOpen, onClose, onSubmit, initialData, isLoading }: PortfolioModalProps) {
  ...
  // Оновлюємо форму при відкритті модалки (для редагування)
  useEffect(() => {
    if (isOpen) {
      reset({
  ...
  // Формуємо FormData при сабміті
  const submitHandler = (data: any) => {
    const formData = new FormData();
    formData.append("title", data.title);
    formData.append("description", data.description);

    // Додаємо файл, якщо він був обраний (ключ "image" згідно вашого завдання)
